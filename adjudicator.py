...
class Adjudicator:
    ...
    def conductBSTM(self, state=[]):
        ...
        def validBuyingSequence(currentPlayer, properties, sign):
            ...
            for propertyObject in properties:
                ...
                if ((currentConstructionsOnProperty + sign * constructions) > 5) or (
                    (currentConstructionsOnProperty + sign * constructions) < 0):
                    return False
        ...
        def monopolyCheck(state, properties, sign):
            ...
            for (propertyId, constructions) in properties:
                ...
                for groupElement in groupElements:
                    ...
                    if groupElementPropertyStatus < (propertyStat - 1) or groupElementPropertyStatus > (
                        propertyStat + 1):
                        return False
        ...
        def handleTrade(agent, otherAgent, cashOffer, propertiesOffer, cashRequest, propertiesRequest):
            ...
            if tradeResponse:
                ...
                mortgagedProperties = list(filter(lambda propertyId: getPropertyStatus(state, propertyId) in [-7, 7],
                                                  propertiesOffer + propertiesRequest))
    ...
    def handle_in_jail_state(self, state, action):
        ...
        if (isinstance(action, tuple) or isinstance(action, list)) and len(action) > 0:
            if action[0] == 'P':
                ...
                if (len(action) > 1) & (
                    action[1] in [self.CHANCE_GET_OUT_OF_JAIL_FREE, self.COMMUNITY_GET_OUT_OF_JAIL_FREE]):
    ...
    def handle_payment_for_agent(self, state, receiver, cash, debt, currentPlayerIndex, otherPlayerIndex,
                                 otherPlayerCash):
        if debt == 0:
    ...
    def handle_payment(self, state):
        ...
        agentOneResult = self.handle_payment_for_agent(state, agentOneReceiver, agentOneCash, agentOneDebt, 1, 2,
                                                       agentTwoCash)
        if agentOneResult:
            agentOneReceiver = 0
            agentOneDebt = 0

        agentTwoResult = self.handle_payment_for_agent(state, agentTwoReceiver, agentTwoCash, agentTwoDebt, 2, 1,
                                                       agentOneCash)
        if agentTwoResult:
            agentTwoReceiver = 0
            agentTwoDebt = 0
    ...
    def isPositionProperty(self, position):
        return (constants.board[position]['class'] == 'Street') or (
        constants.board[position]['class'] == 'Railroad') or (constants.board[position]['class'] == 'Utility')
    ...
    def final_winning_condition(self, state):
        ...
        for i in constants.property_to_space_map:
            ...
            if propertyValue in range(-6, 0):
                agentTwoPropertyWorth += (
                propertyPosition['price'] + ((abs(propertyValue) - 1) * propertyPosition['build_cost']))
            elif propertyValue == -7:
                agentTwoPropertyWorth += (propertyPosition['price'] / 2)
            elif propertyValue in range(1, 7):
                agentOnePropertyWorth += (
                propertyPosition['price'] + ((propertyValue - 1) * propertyPosition['build_cost']))
            elif propertyValue == 7:
    ...
    def runGame(self, agentOne, agentTwo, diceThrows=None, chanceCards=None, communityCards=None):
        ...
        while (self.state[self.PLAYER_TURN_INDEX] < self.TOTAL_NO_OF_TURNS) and (
            (self.diceThrows is None) or (len(self.diceThrows) > 0)):
            ...
            while ((self.diceThrows is None) or (len(self.diceThrows) > 0)):
                ...
                if outOfJail:
                    ...
                    if notInJail:
                        ...
                        log("state",
                            "State after moving the player position and updating state with effect of the position:")
                        stateForLog = list(self.state)
                        stateForLog.pop(7)
                        log("state", stateForLog)
